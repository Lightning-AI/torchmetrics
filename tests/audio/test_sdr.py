...
inputs_1spk = Input(
    preds=torch.rand(3, 2, 1, 8000),
    target=torch.rand(3, 2, 1, 8000),
)
inputs_2spk = Input(
    preds=torch.rand(3, 2, 2, 8000),
    target=torch.rand(3, 2, 2, 8000),
)
...
def test_on_real_audio():
    ...
    assert torch.allclose(
        sdr(torch.from_numpy(deg), torch.from_numpy(ref), rate).float(),
        torch.tensor(0.2211),
        rtol=0.0001,
        atol=1e-4,
    )
