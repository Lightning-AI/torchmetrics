...
def _sklearn_precision_recall_binary(preds, target, sk_fn, ignore_index, multidim_average, zero_division=0):
...
def _sklearn_precision_recall_multiclass(
...
def _sklearn_precision_recall_multilabel_global(preds, target, sk_fn, ignore_index, average, zero_division):
    if average == "micro":
        preds = preds.flatten()
...
def _sklearn_precision_recall_multilabel_local(preds, target, sk_fn, ignore_index, average, zero_division):
    precision_recall, weights = [], []
...
def _sklearn_precision_recall_multilabel(
    preds, target, sk_fn, ignore_index, multidim_average, average, zero_division=0
):
    preds = preds.numpy()
    target = target.numpy()
    ...
    if multidim_average == "global":
        return _sklearn_precision_recall_multilabel_global(preds, target, sk_fn, ignore_index, average, zero_division)
    return _sklearn_precision_recall_multilabel_local(preds, target, sk_fn, ignore_index, average, zero_division)
