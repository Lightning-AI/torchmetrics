...
def test_fid_raises_errors_and_warnings():
    ...
    if _TORCH_GREATER_EQUAL_1_9:
        ...
        with pytest.raises(
            ValueError,
            match="FrechetInceptionDistance metric requires that PyTorch is version 1.9.0 or higher.",
        ):
            _ = FrechetInceptionDistance()
...
def test_not_enough_samples():
    ...
    with pytest.raises(
        RuntimeError,
        match="More than one sample is required for both the real and fake distributed to compute FID",
    ):
        metric.compute()
