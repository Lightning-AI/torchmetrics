...
def test_error_on_invalid_shape(metric_class=SpectralAngleMapper):
    ...
    metric = metric_class()
    with pytest.raises(ValueError, match=r"Expected `preds` and `target` to have BxCxHxW shape\.\*"):
...
def test_error_on_grayscale_image(metric_class=SpectralAngleMapper):
    ...
    metric = metric_class()
    with pytest.raises(ValueError, match=r"Expected channel dimension of `preds` and `target` to be larger than 1\.\*"):
