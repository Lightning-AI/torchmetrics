...
from torchmetrics.image.fd_dinov2 import FrechetDistanceDinoV2, NoTrainDinoV2
...
def test_no_train():
    ...
    class MyModel(Module):
        def __init__(self) -> None:
            super().__init__()
            self.metric = FrechetDistanceDinoV2()
    ...
    assert not model.metric.dinov2.training, "FID metric was changed to training mode which should not happen"


@pytest.mark.skipif(not _TORCH_FIDELITY_AVAILABLE, reason="metric requires torch-fidelity")
def test_fid_pickle():
    """Assert that we can initialize the metric and pickle it."""
    metric = FrechetDistanceDinoV2()
    assert metric
...
def test_fid_raises_errors_and_warnings():
    ...
    if _TORCH_FIDELITY_AVAILABLE:
        with pytest.raises(ValueError, match="Integer input to argument `feature` must be one of .*"):
            _ = FrechetDistanceDinoV2(feature=2)
    else:
        with pytest.raises(
            ModuleNotFoundError,
...
class _DummyFeatureExtractor(Module):
    def __init__(self) -> None:
        ...
        self.extractor = torch.nn.Linear(3 * 224 * 224, 384)
...
@pytest.mark.skipif(not _TORCH_FIDELITY_AVAILABLE, reason="metric requires torch-fidelity")
@pytest.mark.parametrize("feature", [384, 768, 1024, 1536, _DummyFeatureExtractor()])
def test_fid_same_input(feature):
    """If real and fake are update on the same data the fid score should be 0."""
    metric = FrechetDistanceDinoV2(feature=feature)

    seed_all(42)
    for _ in range(2):
        img = torch.randint(0, 255, (10, 3, 224, 224), dtype=torch.uint8)
...
def test_compare_fid(tmpdir, equal_size, feature=768):
    ...
    metric = FrechetDistanceDinoV2(feature=feature).cuda()

    n, m = 100, 100 if equal_size else 90

    # Generate some synthetic data
    seed_all(42)
    img1 = torch.randint(0, 180, (n, 3, 224, 224), dtype=torch.uint8)
    img2 = torch.randint(100, 255, (m, 3, 224, 224), dtype=torch.uint8)
    ...
    torch_fd_dinov2 = calculate_metrics(
    ...
    assert torch.allclose(tm_res.cpu(), torch.tensor([torch_fd_dinov2["frechet_inception_distance"]]), atol=1e-3)
...
def test_reset_real_features_arg(reset_real_features):
    ...
    metric = FrechetDistanceDinoV2(feature=384, reset_real_features=reset_real_features)

    metric.update(torch.randint(0, 180, (2, 3, 224, 224), dtype=torch.uint8), real=True)
    metric.update(torch.randint(0, 180, (2, 3, 224, 224), dtype=torch.uint8), real=False)

    assert metric.real_features_num_samples == 2
    assert metric.real_features_sum.shape == torch.Size([384])
    assert metric.real_features_cov_sum.shape == torch.Size([384, 384])

    assert metric.fake_features_num_samples == 2
    assert metric.fake_features_sum.shape == torch.Size([384])
    assert metric.fake_features_cov_sum.shape == torch.Size([384, 384])
...
def test_normalize_arg(normalize):
    ...
    metric = FrechetDistanceDinoV2(normalize=normalize)
...
def test_antialias():
    ...
    imgs = torch.randint(0, 255, (10, 3, 224, 224), dtype=torch.uint8)

    metric_no_aa = FrechetDistanceDinoV2(feature=384, antialias=False)
    metric_aa = FrechetDistanceDinoV2(feature=384, antialias=True)
