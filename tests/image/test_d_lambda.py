...
for size, channel, p, dtype in [
    ...
    fused = torch.rand(NUM_BATCHES, BATCH_SIZE, channel, size, size, dtype=dtype)
    _inputs.append(
        Input(
            ms=ms,
            fused=fused,
...
@pytest.mark.parametrize(
    "ms, fused, p",
    [(i.ms, i.fused, i.p) for i in _inputs],
)
class TestD_Lambda(MetricTester):
    ...
    def test_d_lambda(self, ms, fused, p, ddp, dist_sync_on_step):
    ...
    def test_d_lambda_functional(self, ms, fused, p):
...
def test_d_lambda_invalid_inputs(ms, fused, p):
    ms_t = torch.rand(ms)
    fused_t = torch.rand(fused, dtype=torch.float64)
