...
class TestMap(unittest.TestCase):
	...
	def test_missing_pred(self):
		"""
		One good detection, one false negative. Map should be lower than 1.
		Actually it is 0.5, but the exact value depends on where we are sampling (i.e. recall's values)
		"""
		gts = [
			dict(
				boxes=torch.Tensor([[10, 20, 15, 25]]),
