...
def _pairwise_linear_distance_update(
...
def _pairwise_linear_distance_compute(distance: Tensor, reduction: Tensor) -> Tensor:
    if reduction == 'mean':
...
def pairwise_linear_distance(
    X: Tensor, Y: Optional[Tensor] = None, reduction: Optional[str] = 'mean', zero_diagonal: bool = True
) -> Tensor:
    distance = _pairwise_linear_distance_update(X, Y, zero_diagonal)
    return _pairwise_linear_distance_compute(distance, reduction)
