...
There have been a few attempts at tackling the reproducibility issues. Papers With Code [@papers_with_code] links research code with its corresponding paper. [ADD ANOTHER EXAMPLE]. However, these methods rely on the paper code made publicly accessible which is not always possible. Our approach is to provide the de-facto reference implementation for metrics. This approach enables non opensource work to still be comparable as long as they’ve used our reference implementations.
