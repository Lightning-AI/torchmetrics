...
def _tschuprows_t_compute(confmat: Tensor, bias_correction: bool) -> Tensor:
    ...
    if bias_correction:
        ...
        n_rows = torch.tensor(n_rows, device=phi_squared.device)
        n_cols = torch.tensor(n_cols, device=phi_squared.device)
        tschuprows_t_value = torch.sqrt(phi_squared / torch.sqrt((n_rows - 1) * (n_cols - 1)))
