...
def _multiclass_stat_scores_update(
    ...
    if multidim_average == "samplewise" or top_k != 1:
        ...
        if ignore_index is not None:
            ...
            preds = preds[idx]
            target = target[idx]
        # Compute per-class statistics using linear memory (O(num_classes))
        # instead of the confusion matrix (O(num_classes**2)).
        # For large num_classes, the confusion matrix approach causes OOM.
