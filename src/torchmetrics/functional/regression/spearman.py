...
def _rank_data(data: Tensor) -> Tensor:
    ...
    The rank refers to the indices of an element in the corresponding
    sorted tensor (starting from 1). Duplicates of the same value will
    be assigned the mean of their rank.
...
def _spearman_corrcoef_update(preds: Tensor, target: Tensor, num_outputs: int) -> Tuple[Tensor, Tensor]:
    """Update and returns variables required to compute Spearman Correlation
    Coefficient.
