...
if not _MATPLOTLIB_AVAILABLE:
    __doctest_skip__ = [
        "BinaryBrier.plot",
        "MulticlassBrier.plot",
        "MultilabelBrier.plot",
    ]
...
class BinaryBrier(Metric):
    ...
    Example (preds is int tensor):
        >>> from torchmetrics.classification import BinaryBrier
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        preds_conf, target_conf = _binary_confusion_matrix_format(preds, target, self.threshold, self.ignore_index)
        confmat = _binary_confusion_matrix_update(preds_conf, target_conf)
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> from torchmetrics.classification import MulticlassBrier
...
class MulticlassBrier(Metric):
    ...
    Example (pred is integer tensor):
        ...
        >>> metric = MulticlassBrier(num_classes=3)
        >>> metric(preds, target)
        tensor([[1, 1, 0],
                [0, 1, 0],
                [0, 0, 1]])
...
class MultilabelBrier(Metric):
    ...
    Example (preds is int tensor):
        >>> from torch import tensor
        >>> from torchmetrics.classification import MultilabelBrier
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
        >>> metric = MultilabelBrier(num_labels=3)
        >>> metric(preds, target)
        tensor([[[1, 0], [0, 1]],
                [[1, 0], [1, 0]],
                [[0, 1], [0, 1]]])
...
class Brier(_ClassificationTaskWrapper):
    r"""Compute the `confusion matrix`_.

    This function is a simple wrapper to get the task specific versions of this metric, which is done by setting the
    ``task`` argument to either ``'binary'``, ``'multiclass'`` or ``'multilabel'``. See the documentation of
    :class:`~torchmetrics.classification.BinaryBrier`,
    :class:`~torchmetrics.classification.MulticlassBrier` and
    :class:`~torchmetrics.classification.MultilabelBrier` for the specific details of each argument influence
    and examples.
    ...
    Legacy Example:
        ...
        >>> confmat = Brier(task="binary", num_classes=2)
        ...
        >>> target = tensor([2, 1, 0, 0])
        >>> preds = tensor([2, 1, 0, 1])
        >>> confmat = Brier(task="multiclass", num_classes=3)
        >>> confmat(preds, target)
        tensor([[1, 1, 0],
                [0, 1, 0],
                [0, 0, 1]])
        ...
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
        >>> confmat = Brier(task="multilabel", num_labels=3)
        >>> confmat(preds, target)
        tensor([[[1, 0], [0, 1]],
                [[1, 0], [1, 0]],
                [[0, 1], [0, 1]]])
    ...
    def __new__(  # type: ignore[misc]
        ...
        **kwargs: Any,
        ...
        if task == ClassificationTask.BINARY:
            return BinaryBrier(threshold, **kwargs)
        if task == ClassificationTask.MULTICLASS:
            if not isinstance(num_classes, int):
                raise ValueError(f"`num_classes` is expected to be `int` but `{type(num_classes)} was passed.`")
            return MulticlassBrier(num_classes, **kwargs)
        if task == ClassificationTask.MULTILABEL:
            if not isinstance(num_labels, int):
                raise ValueError(f"`num_labels` is expected to be `int` but `{type(num_labels)} was passed.`")
            return MultilabelBrier(num_labels, threshold, **kwargs)
